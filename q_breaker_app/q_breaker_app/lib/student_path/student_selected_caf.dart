...
class StudentSelectedCafeteria extends StatefulWidget {
  ...
  @override
  State<StudentSelectedCafeteria> createState() =>
      _StudentSelectedCafeteriaState();
}
...
class _StudentSelectedCafeteriaState extends State<StudentSelectedCafeteria> {
  ...
  void initState() {
    ...
    () async {
      ...
      await fetchUserData(
          mycurrentUser!.id.toString(), context.read<UserProvider>());
    };
    final currentCaf =
        Provider.of<CafProvider>(context, listen: false).currentUser;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Center(
            child: Text(
                'Thank you for selecting ${currentCaf!['name'] ?? 'Loading'}')),
        centerTitle: true,
                  ...
                  children: [
                    ...
                    TextButton(
                        onPressed: () {
                          ...
                          issueReceipt(
                              currentUser['Today\'s balance'].toDouble(),
                              currentCost,
                              currentCaf.foods,
                              currentUser['Name'],
                              currentCaf['name'],
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            for (Food food in currentCaf.foods) MyFoodCard(food: food),
          ],
        ),
      ),
    );
  ...
  void issueReceipt(double currentBalance, double evaluatedCost,
      List<Food> foodList, String studentName, String cafName, int studentId) {
